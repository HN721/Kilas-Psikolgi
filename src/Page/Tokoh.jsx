    ...
    <div className="mx-2 lg:mx-16 mt-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4">
        {Tokoh.map(person => (
          ...
          <img className="w-56 h-56 lg:w-full lg:h-64 object-cover" src={person.gambar} alt={person.nama} />
          <h1 className="font-sans text-xl lg:text-2xl mt-4 font-semibold">{person.nama}</h1>
          ...
          <button onClick={() => handleSelect(person.id)}  className="w-full bg-black hover:bg-button p-3 rounded-md text-slate">Lihat Teori</button>
        </div>
        ))}
      </div>
    </div>
